...
- **Input**: `type: "ELIMINATE", payload: { playerIds: ["id1"], points?: 50, winnerPoints?: 100 }`
   - `points` — optional points awarded to each **eliminated** player.
   - `winnerPoints` — optional points awarded to each **surviving** player after elimination.
- **Broadcast**: `PLAYERS_UPDATE` sent to all (with updated points for both losers and survivors).
...
#### [EMIT] `DECLARE_NEITHER` (via `GAME_EVENT`) (Host Only — Final VIDEO Round Only)

- **Purpose**: Allows the host to reject **both** finalists in the final VIDEO round, ending the game with no winner.
- **Input**: `type: "DECLARE_NEITHER", payload: {}`
- **Guards**: Server will reject this event with an error if:
   - The current round is not a `VIDEO` round.
   - There are not exactly 2 remaining (non-eliminated) players.
- **Broadcast**: `GAME_ENDED` is sent to everyone with `{ winner: null, noWinner: true }`.
- **Side Effect**: Both finalists are marked as `isEliminated: true`. The game is finalized in MongoDB with `winnerId: null`.
...
| `GAME_ENDED`        | `{ winner: Player \| null, noWinner?: boolean }` — `noWinner: true` when host chose "Neither" | Everyone      |
